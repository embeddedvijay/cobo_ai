...
function renderTimingTable() {
  ...
  editor.innerHTML = `<div class="edit-head"><div class="edit-icon">＋</div><div><h2>Add / Edit Market</h2><p>Create or update market timings</p></div></div>
    ...
    <div class="dynamic-timing-box" style="display:block"><div class="mini-title">Dynamic Timing — post-close grace</div><p class="editor-note">Normal parsed game only. After End Time, accept only within these limits.</p><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:12px 0"><label class="cutting-label">Enable<input data-dynamic="enabled" type="checkbox" ${dynamic.enabled ? 'checked' : ''}></label><label style="display:grid;gap:6px;color:#afbee0;font-size:11px;font-weight:800">Extra Minutes<input data-dynamic="minutes" type="number" min="0" max="30" value="${esc(dynamic.minutes ?? 0)}"></label></div><div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px">${[['ank','ANK'],['jodi','JODI'],['sp','SP'],['dp','DP'],['tp','TP']].map(([key,label])=>`<label style="display:grid;gap:6px;color:#afbee0;font-size:11px;font-weight:800">${label} limit<input data-dynamic="${key}" type="number" min="0" value="${esc(dynamic[key] ?? 0)}"></label>`).join('')}</div></div>
